...
import getElementScroll from "../dom/getElementScroll";
 ...
 * @param {number} [throttleValue=0]
 * @returns {SubscriberDefinition}
 */
function createSubscriberForDOMElement(element, throttleValue = 0) {
    ...
    if (throttleValue || (throttleValue === 0 && !existingData.throttlers[throttleValue])) {

        // Make sure we have a callback array for the given throttle value.
        if (!Array.isArray(existingData.callbacks[throttleValue])) {
            existingData.callbacks[throttleValue] = [];
        }

        // Make sure we have a throttled version of the base observable
        if (throttleValue && !existingData.throttlers[throttleValue]) {
            existingData.throttlers[throttleValue] = existingData.baseObservable.throttleTime(throttleValue, animationFrame, { trailing: true });
        }

        // Make sure we have a subscriber for when something happens for the given throttle value.
        if (!existingData.subscribers[throttleValue]) {
            existingData.subscribers[throttleValue] =
                // Use a throttled observer if a throttle value is given, or the base observable if not.
                (throttleValue ? existingData.throttlers[throttleValue] : existingData.baseObservable)
                    // Subscribe to the thing!
                    .subscribe(event => {
                        const { top, left } = getElementScroll(element);
                        existingData.callbacks[throttleValue].forEach(funcRef => funcRef({event, top, left }));
                    });
        }
    }
 ...
 * @param {number} [throttleValue=0] - Optional throttle value, given in milliseconds. If omitted, no throttling is employed.
 *
 ...
 */
export function onScroll(elementList, callbackList, throttleValue = 0) {

    forEach(elementList, element => {
        if (isElement(element)) {

            // Make sure we have an observable to work with
            const baseData = createSubscriberForDOMElement(element, throttleValue);

            // Add all given callbacks to the element's observable
            forEach(callbackList, callback => baseData.callbacks[throttleValue].push(callback));
 ...
 * @param {number} [throttleValue=-1] The specific throttle to remove the callback(s) for. Defaults to -1 and not 0, because a zero-value just means no throttle.
 *
 *
 ...
 */
export function removeScrollCallback(elementList, callbackListToRemove, throttleValue = -1) {
    forEach(elementList, element => {
        if (isElement(element)) {
            ...
            if (baseData && typeof baseData === "object") {
                ...
                if (throttleValue > -1) {
                    ...
                    if (callbackListToRemove) {
                        forEach(callbackListToRemove, callback => {
                            if (Array.isArray(baseData.callbacks[throttleValue])) {
                                // Filter the callbacks to only include those that don't match the given callback
                                baseData.callbacks[throttleValue] = filter(baseData.callbacks[throttleValue], funcRef => funcRef !== callback);
                            }
                        });
                    } else {
                        // If we're here, no specific callbacks were given. Just remove all of them.
                        baseData.subscribers[throttleValue].unsubscribe();
                        delete baseData.callbacks[throttleValue];
                        delete baseData.subscribers[throttleValue];
                    }
                } else {
                    ...
                    if (callbackListToRemove) {
                        ...
                        forEach(callbackListToRemove, callback => {
                            forEach(baseData.callbacks, (callbackList, throttleIdx) => {
                                baseData.callbacks[throttleIdx] = filter(callbackList, funcRef => funcRef !== callback);
                            });
                        });
                    } else {
                ...
                if (remainingCallbacks === 0) {
                    forEach(baseData.subscribers, subscription => subscription.unsubscribe());
                    baseData = null;
                }
...
/**
 * Exporting default because reasons.
 */
