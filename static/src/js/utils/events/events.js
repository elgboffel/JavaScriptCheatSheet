 ...
 *
 * @param {Element|HTMLElement|NodeList|Window|Document} elements     - The DOM element(s) we'll be working on.
 * @param {string} eventTypes               - A string containing one or more event names to remove, separated by space.
                ...
                if (event) {
                    if (eventHandlerToRemove !== undefined) {
                        ...
                        if (Array.isArray(callbackList) && callbackList.length > 0) {
                            callbackList = filter(callbackList, callbackObject => callbackObject.eventHandler !== eventHandlerToRemove);
 ...
 *
 * @param {Element|HTMLElement|NodeList|Window|Document} elements - The element(s) to clean up.
 */
 ...
 *
 * @param {Element|HTMLElement|NodeList|Window|Document} elements - The element(s) we'll be working on.
 * @param {string} eventTypes           - A string containing one or more events to add (ie. "click", "mouseenter" etc.), separated by space.
 ...
 */
export function addEvent(elements, eventTypes, eventHandler, useCapture = false, once = false) {
 ...
 * @param {boolean} [useCapture=false]  - Whether or not to use event capturing. See JS-docs for more.
 */
export function addEventOnce(elements, eventTypes, eventHandler, useCapture = false) {
 ...
 * @param {HTMLElement|HTMLDocument|Window|Document} elementScope    - Parent DOM-element to set eventListener on (optional, defaults to document).
 */
export function delegateEvent(selector, eventTypes, eventHandler, elementScope = document) {
