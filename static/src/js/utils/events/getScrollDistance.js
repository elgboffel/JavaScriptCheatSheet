 ...
 * but for touch it's more complicated, and we have to rely on the startY ("last known offset") and the current offset.
 *
 * @param {TouchEvent|MouseEvent} event
 * @param {number} startY - Last known offset
 * @returns {number} Scroll distance - negative is down, positive up
 */
export function getScrollDistance(event, startY) {
    let delta;
    ...
    if (!isTouchEvent(event)) {
        ...
        delta = Math.max(-1, Math.min(1, val - startY));
    }
