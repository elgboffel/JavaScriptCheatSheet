...
if (document.documentMode && document.documentMode <= 11) {
    ...
    (function () {
        function CustomEvent (event, params) {
            const useParams = params || { bubbles: false, cancelable: false, detail: undefined };
            const evt = document.createEvent('CustomEvent');
            evt.initCustomEvent(event, useParams.bubbles, useParams.cancelable, useParams.detail);
            return evt;
        }
...
export function triggerCustomEvent(target, eventName, eventData = {}) {
    ...
    forEach(target, element => {
        ...
        if (window.CustomEvent) {
            event = new CustomEvent(eventName, { detail: eventData, bubbles: true, cancelable: true });
        } else {
