 ...
 */
export function getCachedContent(matchPattern, contentType, cacheName = "fetches") {
        ...
        return caches.open(cacheName).then(cache =>
            cache.keys().then(keys => {
                ...
                keys.forEach(cachedItem => {
                    promises.push(cache.match(cachedItem).then(response => {
                        if (
                            (!contentType || response.headers.get('Content-Type').includes(contentType)) &&
                            (!matchPattern || response.url.match(matchPattern))
                        ) {
