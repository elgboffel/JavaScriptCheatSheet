 ...
 * @param {boolean} [parseCookie=true] - Set to *false* if you want possible numbers, JSON objects and arrays returned as a string.
 * @returns {string|number|Object|Array|boolean} Cookie value.
 */
export function getCookie(name, parseCookie = true) {
    ...
    forEach(cookies, cookie => {
        ...
        if (!name || typeof cookieJar[name] === 'undefined') {
            ...
            if (!parseCookie && cookieValue.charAt(0) === '"') {
...
export function setCookie(name, value, attributes = {}) {
    ...
    if (typeof attributes.expires === 'number') {
        attributes.expires = new Date(new Date() * 1 + attributes.expires * 864e+5); // 864e+5 = 86400000 ms = 24 hours
    }
    ...
    for (const attributeName in cookieAttributes) {
        ...
        attributesString += `=${String(attributes[attributeName]).split(';')[0]}`;
    }
